...
static int __init nwhw_config(void)
{
	...
	while ((ndev < NWHW_MAX_DEV) &&
	       (dev = __dev_get_by_name(&init_net, nwhwdev[ndev].user_dev_name))) {
		...
		if (valid_ether) {
			if ((!dev->netdev_ops->ndo_set_mac_address ||
			     dev->netdev_ops->ndo_set_mac_address(dev, &ether_addr))) {
				printk(KERN_WARNING
...
{
        char sstr[COMMAND_LINE_SIZE]; /* Save str to prevent strsep below to destroy it */
	char *sstr_ptr = sstr;
	char *opt;
	int j = 0;
	...
	if (!str || !*str)
		return 0;
	strcpy(sstr, str);
	while (((opt = strsep(&sstr_ptr, ";")) != NULL) && (j < NWHW_MAX_DEV)) {
		char *p;
